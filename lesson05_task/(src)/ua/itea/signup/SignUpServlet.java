...
public class SignUpServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			boolean added = false;
			if(suh.isCorrect() && (added = usersDB.addUser(connector.getConnection(), sub.createUser()))) {
	...
	private void showSignUpSuccessForm(HttpServletResponse resp) throws IOException {
		resp.sendRedirect("/lesson05_task/sign-up-success.jsp");
	}

	private void showSignUpForm(HttpServletResponse resp) throws IOException {
		resp.sendRedirect("/lesson05_task/sign-up.jsp");
	}
}
